...
export default function TallerCard({ taller }: { taller: Taller }) {
  const mensajeWhatsapp = encodeURIComponent(
    `Hola, quiero pedir un turno en ${taller.nombre}.`
  );

  return (
    <div className="flex flex-col rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <span className="rounded-full border border-umarti-navy px-3 py-1 text-xs font-semibold text-umarti-navy">
          {taller.especialidad}
        </span>
        {taller.verificado && (
          <span
      ...
      <h3 className="mt-3 font-semibold text-umarti-navy">{taller.nombre}</h3>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {taller.marcasQueAtiende.map((marca) => (
          <span
            key={marca}
      ...
      <a
        href={`https://wa.me/${taller.whatsapp}?text=${mensajeWhatsapp}`}
        target="_blank"
